...
# EKS defaults (ENV=eks). RAM keeps Azure OpenAI via Entra; AZURE_RESOURCE_GROUP
# and RAM_IDENTITY_NAME point at the existing UAMI from the AKS deployment.
EKS_CLUSTER_NAME="${EKS_CLUSTER_NAME:-ram-eks}"
EKS_AWS_REGION="${EKS_AWS_REGION:-us-east-1}"
EKS_KUBERNETES_VERSION="${EKS_KUBERNETES_VERSION:-1.31}"
EKS_NODE_INSTANCE_TYPE="${EKS_NODE_INSTANCE_TYPE:-m5.xlarge}"
EKS_NODE_COUNT="${EKS_NODE_COUNT:-3}"
AZURE_RESOURCE_GROUP="${AZURE_RESOURCE_GROUP:-${AKS_RESOURCE_GROUP}}"
RAM_IDENTITY_NAME="${RAM_IDENTITY_NAME:-ram-aoai-identity}"
RAM_SERVICE_ACCOUNT_NAME="${RAM_SERVICE_ACCOUNT_NAME:-redis-agent-memory}"
